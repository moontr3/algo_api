...
| `sharing_mode` | `str` | Видимость проекта.<br>`algo_api.SHARING_MODE_PRIVATE` - виден только создателю<br>`algo_api.SHARING_MODE_GROUP` - виден одногруппникам<br>`algo_api.SHARING_MODE_PUBLIC` - виден всем |
