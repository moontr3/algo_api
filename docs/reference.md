...
Возвращает код проекта как объект класса `str`.

Работает только с **вашими** Python проектами.

Если указаннй проект не является Python проектом, поднимет ошибку `TypeError`.

Если исходный код проекта не найден (в основном это происходит потому что он не пренадлежит вам), поднимет ошибку.
