...
Если проект не найден или не доступен, поднимет ошибку `InaccessableProject`

Если проект не имеет тип `python`, поднимет ошибку `TypeError`

Если код проекта не найден (в основном потому что он не принадлижит вам), поднимет ошибку `InaccessableProject`
