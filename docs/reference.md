...
> Некоторых атрибутов, доступных через `dict` у некоторых обьектов, может не существовать в связи с неизвестностью обозначения атрибута.
...
| Имя | Тип | Описание |
|-----|-----|-----|
| `login_name` | `str` | Ваш логин в системе. |
| `password` | `str` | Ваш пароль в системе. |
| `id` | `int` | ID пользователя на платформе. |
...
Возвращает профиль пользователя под указанным ID.
